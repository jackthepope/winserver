...
var run_cmd = '';
let nomax = { 'calc': 0 , 'notepad-fonts': 0, 'camera-notice': 0};
...
let cms = {
    ...
    'desktop.icon': [
        ...
        arg => {
            if (arg[1] >= 0) {
                return ['<i class="bi bi-trash3"></i> Delete', 'desktopItem.splice(' + (arg[1]) + ', 1);saveDesktop();setIcon();addMenu();'];
            } else {
                return 'null';
            }
        }
...
let dps = {
    'notepad.file': [
        ...
        ['<i class="bi bi-box-arrow-right"></i> Save As', `hidedp(true);$('#win-notepad>.save').attr('href', window.URL.createObjectURL(new Blob([$('#win-notepad>.text-box').html()])));$('#win-notepad>.save')[0].click();`],
        '<hr>',
        ['<i class="bi bi-x"></i> Exit', 'isOnDp=false;hidedp(true);hidewin(\'notepad\')'],
    ],
    'notepad.edit': [
    ...
    'notepad.view': [
        ['<i class="bi bi-type"></i> Insert Normal Text', 'hidedp(true);$(\'#win-notepad>.text-box\')[0].innerHTML+=\'<p>T</p>\''],
        ['<i class="bi bi-type-h1"></i> Insert Main Title', 'hidedp(true);$(\'#win-notepad>.text-box\')[0].innerHTML+=\'<h1>H1</h1>\''],
        ['<i class="bi bi-type-h2"></i> Insert Secondary Title', 'hidedp(true);$(\'#win-notepad>.text-box\')[0].innerHTML+=\'<h2>H2</h2>\''],
        ...
        ['<i class="bi bi-type-italic"></i> Insert Italic', 'hidedp(true);$(\'#win-notepad>.text-box\')[0].innerHTML+=\'<i>I</i>\''],
        ['<i class="bi bi-type-bold"></i> Insert Bold', 'hidedp(true);$(\'#win-notepad>.text-box\')[0].innerHTML+=\'<b>B</b>\''],
        '<hr>',
        ['<i class="bi bi-fonts"></i> Fonts', 'font_window=true;hidedp(true);showwin(\'notepad-fonts\');apps.notepadFonts.reset();'],
    ]
};
...
-r: Restart
-f: Log off
-a: Cancel the previous operation
-t time: Specify the operation to occur after time seconds

`(inTerminal?'' : `
Press any key to continue.&nbsp;.&nbsp;.<input type="text" onkeydown="hidewin('terminal')"></input>`));
...
let apps = {
                ...
                res.json().then(json => {
                    ...
                    if (sha != json[0].sha) {
                        ...
                        if (msg.match(/v[0-9]*\.[0-9]*\.[0-9]*/)) {
                            ...
                            window.setTimeout(() => {
                                $('#win-setting>.page>.cnt.update>.lo>.update-main .notice')[0].innerText = 'Updates are available';
                                $('#win-setting>.page>.cnt.update>.lo>.update-main .detail')[0].innerText = `Currently the latest version: ${msg}`;
                            ...
                            window.setTimeout(() => {
                                let da = new Date();
                                $('#win-setting>.page>.cnt.update>.lo>.update-main .notice')[0].innerText = 'Windows Currently, it is the latest version';
                                $('#win-setting>.page>.cnt.update>.lo>.update-main .detail')[0].innerText = `Last checked time: ${da.toLocaleDateString()}，${da.toLocaleTimeString()}`;
                                $('#win-setting>.page>.cnt.update>.lo>.update-main>div:last-child').removeClass('disabled');
                                $('#win-setting>.page>.cnt.update>.setting-list>.update-now>div>p:first-child')[0].innerText = 'No updates available';
                                $('#win-setting>.page>.cnt.update>.setting-list>.update-now>div>p:last-child')[0].innerText = 'Windows is currently the latest version';
                            }, 6000);
                        }
                    }
                });
    ...
    box-shadow: 0 1px 2px var(--s3d);
            ...
            </div><p class="info">32.6 GB available, total 143 GB</p></div></a><a class="a item act" ondblclick="apps.explorer.goto('D:')" ontouchend="apps.explorer.goto('D:')"
            ...
            </div><p class="info">185.3 GB available, total 216 GB</p></div></a></div>`;
        ...
        paste: (path) => {
            ...
            if (pathl[pathl.length - 1] == 'C:') {
                ...
                $('#win-explorer>.path>.tit>.icon')[0].style.backgroundImage = 'url("https://i.imgur.com/8OylhZw.png")';
            }
        ...
        goto: (path, clear = true) => {
            ...
            if (pathl[pathl.length - 1] == 'C:') {
                ...
                m_tab.rename('explorer', '<img src="https://i.imgur.com/8OylhZw.png" style="margin-top:2.5px">' + pathl[pathl.length - 1]);
            }
            else if (pathl[pathl.length - 1] == 'D:') {
                $('#win-explorer>.path>.tit>.icon')[0].style.backgroundImage = 'url("apps/icons/explorer/disk.svg")';
...
Micrȯsoft Windows [Version 12.0.39035.7324]
(c) Microsoft Corporation. All rights reserved.
    ...
    edge: {
        ...
        newtab: () => {
            m_tab.newtab('edge', 'Microsoft Edge');
...
// 日期、时间
let da = new Date();
let date = `Today is ${['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][da.getDay()]}, ${da.getFullYear()}-${(da.getMonth() + 1).toString().padStart(2, '0')}-${da.getDate().toString().padStart(2, '0')}`;$('#s-m-r>.row1>.tool>.date').text(date);
...
function loadtime() {
    let d = new Date();
    // Set options to exclude seconds
    let options = { hour: '2-digit', minute: '2-digit' };
    let time = d.toLocaleTimeString([], options); // Empty array for locale, options for formatting
